...
class PreprocessContext {
  ...
  getContext(indent) {
    ...
    indent = indent || 0;
    r = "".padEnd(indent, " ");
    r += this.slice.getLine(this.lineNumber - this.slice.parentLine);
...
const BuiltinPreprocessError = Object.freeze({
  UNEXPECTED: new DynamicPreprocessErrorBuilder(PreprocessError, token => "unexpected \"" + token.content.content + "\"."),
  ...
  NO_MACRO_NAME: new SimplePreprocessErrorBuilder(PreprocessError, "no macro name given in #define directive."),
  INVALID_MACRO_NAME: new SimplePreprocessErrorBuilder(PreprocessError, "macro names must be identifiers."),
});
